...
public class Main {
  ...
  static {
    try {
      CONF_FILE_URI = ClassLoader.getSystemResource("dist/conf/simulator.conf").toURI();
      OUT_FILE_URI = CONF_FILE_URI.resolve(new URI("../output/"));
      System.out.println(OUT_FILE_URI);
    ...
    try {
      ...
      for (Block b : blockList) {
        if (!orphans.contains(b)) {
          pw.println("OnChain : " + b.getHeight() + " : " + b + "   parents  :   "+b.getParent()+" : "+b.getParent().getHeight());
        } else {
          pw.println("Orphan : " + b.getHeight() + " : " + b + "    parents  :   "+b.getParent()+" : "+b.getParent().getHeight());
        }
      }
      pw.close();
